// Thin wrapper around the Telegram MainButton. Used for primary actions
// (e.g. saving attendance) so the button lives in the native header area.
...
export function showMainButton(text: string, onClick: MainButtonHandler) {
  ...
  if (!mb) return
  mb.setParams?.({ text, color: getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-button-color').trim() || '#2481cc' })
  mb.show?.()
